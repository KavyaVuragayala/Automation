...
public class P3_RestAssured_TestNG_AssertJ_Assertions {
    ...
    public void test_POST() {
        ...
        // ------------------TestNG / AsserJ
        // 1. need to extract the data

        bookingId = response.then().extract().path("bookingid");
        String firstname = response.then().extract().path("booking.firstname");
        String lastname = response.then().extract().path("booking.lastname");

        // 2. extraction can be done using ----------------JSON Path for keys and values
        ...
        assertThat(jp.getString("firstname")).isEqualTo("Alice");
        assertThat(jp.getString("booking.lastname")).isEqualTo("John");
